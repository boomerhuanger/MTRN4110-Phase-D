...
void findParentVertices(std::vector<int> parentVertices[], int startVertex, int finishVertex, std::vector<int> graph[]) {
    ...
    //used to store the distance from the start vertex to each vertex in the map and initialise all distances to be infinity 
    //(actually highest value possible in C++ but effectively infinity)
    const auto INFINITY = std::numeric_limits<int>::max();
    int distanceFromStart[TOTAL_CELLS];
    std::fill_n(distanceFromStart, TOTAL_CELLS, INFINITY);
...
//once you have reached -infinity as a parent node (as set above), the shortest path has been found 
...
int main(int argc, char **argv) {
    ...
    for (std::vector<int>::size_type i = 0; i < shortestPathWithLeastTurns.size(); i += 1) {
    ...
    std::ofstream pathPlanForWriting{PATH_PLAN_FILE_NAME, std::ios::trunc};  
